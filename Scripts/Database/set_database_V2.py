...
# Avoid overlapp of labels and combine Larynx and Trachea 
...
for i in range(len(list1)):
    ...
    # Combine Larynx and Trachea only if both are present                      
    if larynx != 'none' and trachee != 'none':
        image_larynx = itk.imread(larynx)
        image_larynx1 = gt.applyTransformation(input=image_larynx, like=image_ref, force_resample=True, interpolation_mode='NN')
        image_larynx_np = itk.array_view_from_image(image_larynx1)
        mask = image_larynx_np>0
        mask_oar[mask] = dt.Label['Larynx-Trachee']
	
        image_trachee = itk.imread(trachee)
        image_trachee1 = gt.applyTransformation(input=image_trachee, like=image_ref, force_resample=True, interpolation_mode='NN')
        image_trachee_np = itk.array_view_from_image(image_trachee1)
        mask = image_trachee_np>0
        mask_oar[mask] = dt.Label['Larynx-Trachee']

        OAR.append('Larynx-Trachee') # To avoid multi label for same OAR
        nb_oar[dt.Label['Larynx-Trachee']-1] += 1 
